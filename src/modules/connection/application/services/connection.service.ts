...
export class ConnectionService implements InterfaceConnectionUseCase {
  ...
  async getConnectionById(connectionId: string): Promise<ConnectionResponse | null> {
    try {
      ...
      const verified = await this.connectionRepository.verifyConnectionExists(connectionId);
      if (!verified) {
        throw new RpcException({
  ...
  async findAllConnections(limit: number, offset: number): Promise<ConnectionResponse[]> {
    try {
      const connections = await this.connectionRepository.findAllConnections(limit, offset);
  ...
  async createConnection(connection: CreateConnectionRequest): Promise<ConnectionResponse | null> {
    try {
      ...
      const verified = await this.connectionRepository.verifyConnectionExists(connection.connectionId);
      if (verified === true) {
        throw new RpcException({
      ...
      const connectionModel: ConnectionModel = ConnectionMapper.fromCreateConnectionRequestToConnectionModel(connection);

      const newConnection = await this.connectionRepository.createConnection(connectionModel);
  ...
  async updateConnection(connectionId: string, connection: Partial<UpdateConnectionRequest>): Promise<ConnectionResponse | null> {
    try {
  ...
  async findConnectionAndPropertyByCadastralKey(propertyCadastralKey: string): Promise<ConnectionAndPropertyResponse | null> {
    try {
      ...
      const connectionAndProperty = await this.connectionRepository.findConnectionAndPropertyByCadastralKey(propertyCadastralKey);
